...
@Component({
  selector: 'app-total-hardness',
  templateUrl: './total-hardness.component.html',
  styleUrls: ['./total-hardness.component.scss']
})
export class TotalHardnessComponent implements AfterViewInit, OnDestroy {
  ...
  private initMap(): void {
    this.map = L.map('map_total_hardness', {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Total Hardness']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Total Hardness'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Total Hardness:</span>
        <span class="text-blue-400" [ngClass]="(data['Total Hardness'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Total Hardness'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average Total Hardness by District';
    const xLabel = 'Districts';
    const yLabel = 'Avg. Total Hardness';
    ...
    // Sort the data in ascending order based on Total Hardness
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.mean(data, d => +d['Total Hardness'])
    }));
    
    // Sort the districts in ascending order based on average Total Hardness
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerTotalHardness')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Total Hardness']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Total Hardness']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Total Hardness']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Total Hardness']}`)
