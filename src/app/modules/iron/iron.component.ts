...
@Component({
  ...
  styleUrls: ['./iron.component.scss']
})
export class IronComponent implements AfterViewInit, OnDestroy {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Iron']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Iron'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Iron:</span>
        <span class="text-blue-400" [ngClass]="(data['Iron'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Iron'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average Iron by District';
    const xLabel = 'Districts';
    const yLabel = 'Avg. Iron';
    ...
    // Sort the data in ascending order based on Iron
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.mean(data, d => +d['Iron'])
    }));
    
    // Sort the districts in ascending order based on average Iron
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerIron')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Iron']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Iron']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Iron']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Iron: ${d['Iron']}`)
