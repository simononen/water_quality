...
@Component({
  selector: 'app-tupidity',
  templateUrl: './tupidity.component.html',
  styleUrls: ['./tupidity.component.scss']
})

export class TupidityComponent implements AfterViewInit, OnDestroy {
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Turbidity'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Turbidity:</span>
        <span class="text-blue-400" [ngClass]="(data['Turbidity'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Turbidity'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average Turbidity by District';
    const xLabel = 'Districts';
    const yLabel = 'Avg. Turbidity';
    ...
    // Sort the data in ascending order based on Turbidity
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.sum(data, d => +d['Turbidity'])
    }));
    
    // Sort the districts in ascending order based on average Turbidity
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerTupidity')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Turbidity']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Turbidity']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Turbidity']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Turbidity']}`)
