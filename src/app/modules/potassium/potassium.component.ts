...
@Component({
  selector: 'app-potassium',
  templateUrl: './potassium.component.html',
  styleUrls: ['./potassium.component.scss']
})
export class PotassiumComponent implements AfterViewInit, OnDestroy {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Potassium']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Potassium'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Potassium:</span>
        <span class="text-blue-400" [ngClass]="(data['Potassium'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Potassium'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average Potassium by District';
    const xLabel = 'Districts';
    const yLabel = 'Avg. Potassium';
    ...
    // Sort the data in ascending order based on Potassium
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.mean(data, d => +d['Potassium'])
    }));
    
    // Sort the districts in ascending order based on average Potassium
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerPotassium')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Potassium']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Potassium']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Potassium']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Potassium']}`)
