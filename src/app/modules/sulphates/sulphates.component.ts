...
@Component({
  selector: 'app-sulphates',
  templateUrl: './sulphates.component.html',
  styleUrls: ['./sulphates.component.scss']
})
export class SulphatesComponent implements AfterViewInit, OnDestroy {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Sulphates']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Sulphates'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Sulphates:</span>
        <span class="text-blue-400" [ngClass]="(data['Sulphates'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Sulphates'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average Sulphates by District';
    const xLabel = 'Districts';
    const yLabel = 'Avg. Sulphates';
    ...
    // Sort the data in ascending order based on Sulphates
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.mean(data, d => +d['Sulphates'])
    }));
    
    // Sort the districts in ascending order based on average Sulphates
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerSulphates')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Sulphates']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Sulphates']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Sulphates']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Sulphates']}`)
