...
export class LeadComponent implements AfterViewInit, OnDestroy {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Lead']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Lead'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Lead:</span>
        <span class="text-blue-400" [ngClass]="(data['Lead'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Lead'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average Lead by District';
    const xLabel = 'Districts';
    const yLabel = 'Avg. Lead';
    ...
    // Sort the data in ascending order based on Lead
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.mean(data, d => +d['Lead'])
    }));
    
    // Sort the districts in ascending order based on average Lead
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerLead')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Lead']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Lead']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Lead']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Lead']}`)
