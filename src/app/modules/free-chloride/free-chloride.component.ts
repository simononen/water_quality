...
@Component({
  selector: 'app-free-chloride',
  templateUrl: './free-chloride.component.html',
  styleUrls: ['./free-chloride.component.scss']
})
export class FreeChlorideComponent implements AfterViewInit, OnDestroy {
  ...
  private initMap(): void {
    this.map = L.map('map_FreeChloride', {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Free Chlorine']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Free Chlorine'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Free Chlorine:</span>
        <span class="text-blue-400" [ngClass]="(data['Free Chlorine'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Free Chlorine'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average Free Chlorine by District';
    const xLabel = 'Districts';
    const yLabel = 'Avg. Free Chlorine';
    ...
    // Sort the data in ascending order based on Free Chlorine
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.mean(data, d => +d['Free Chlorine'])
    }));
    
    // Sort the districts in ascending order based on average Free Chlorine
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerFreeChloride')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Free Chlorine']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Free Chlorine']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Free Chlorine']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Free Chlorine']}`)
