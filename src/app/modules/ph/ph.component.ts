...
export class PhComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], {
        radius: this.scaledRadius(item['pH'], maxPh),
      });
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average pH Level by District';
    ...
    // Sort the data in ascending order based on pH
    const districtAverages = data.map((d) => ({
      district: d.District,
      average: d3.mean(data, (d) => +d['pH']),
    }));

    // Sort the districts in ascending order based on average pH
    districtAverages.sort((a: any, b: any) => a.average - b.average);

    const svg = d3
      .select('#chartContainerPh')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const yScale = d3
      .scaleLinear()
      .domain([0, d3.max(pHData) as number])
    ...
    chart
      ...
      .on('mouseover', function (event, d) {
        ...
        tooltip
          .style('opacity', 1)
          .html(
            `<strong>${d.District}</strong><br>Average pH: ${d['pH']}`
          )
  ...
  createGaugeChart(numDistricts: number, averagePH: number): void {
    ...
    svg
      .append('path')
      .datum({ endAngle: calculateAngle(averagePH) })
      .style('fill', '#4682b4')
  ...
  createChart(dataSrc: any): void {
    const container = d3.select('#chartContainerPh');
    const width = 1000;
    const height = 400;
