...
@Component({
  selector: 'app-totaldissolvedsolids',
  templateUrl: './totaldissolvedsolids.component.html',
  styleUrls: ['./totaldissolvedsolids.component.scss']
})
export class TotaldissolvedsolidsComponent implements AfterViewInit, OnDestroy {
  ...
  private initMap(): void {
    this.map = L.map('map_total_dissolved_solids', {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Total dissolved solids']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Total dissolved solids'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Total dissolved solids:</span>
        <span class="text-blue-400" [ngClass]="(data['Total dissolved solids'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Total dissolved solids'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Total dissolved solids by District';
    const xLabel = 'Districts';
    const yLabel = 'Total dissolved solids';
    ...
    // Sort the data in ascending order based on Total dissolved solids
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.sum(data, d => +d['Total dissolved solids'])
    }));
    
    // Sort the districts in ascending order based on average Total dissolved solids
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerTotalDissolvedSolids')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Total dissolved solids']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Total dissolved solids']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Total dissolved solids']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Total dissolved solids']}`)
