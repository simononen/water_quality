...
@Component({
  selector: 'app-magnesium-hardness',
  templateUrl: './magnesium-hardness.component.html',
  styleUrls: ['./magnesium-hardness.component.scss']
})
export class MagnesiumHardnessComponent implements AfterViewInit, OnDestroy {
  ...
  private initMap(): void {
    this.map = L.map('map_magnesium_hardness', {
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Electrical Conductivity'], maxPh) });
  ...
  createBarChart(data: any[]): void {
    ...
    const svg = d3.select('#chartContainermagnesiumHardness')
      .append('svg')
      .attr('width', svgWidth)
    ...
    chart.selectAll('.bar')
      ...
      .attr('height', d => chartHeight - yScale(+d['Electrical Conductivity']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Electrical Conductivity']}`)
