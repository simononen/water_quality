...
@Component({
  selector: 'app-nitrates',
  templateUrl: './nitrates.component.html',
  styleUrls: ['./nitrates.component.scss']
})
export class NitratesComponent implements AfterViewInit, OnDestroy {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Nitrates']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Nitrates'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Nitrates:</span>
        <span class="text-blue-400" [ngClass]="(data['Nitrates'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Nitrates'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Average Nitrates by District';
    const xLabel = 'Districts';
    const yLabel = 'Avg. Nitrates';
    ...
    // Sort the data in ascending order based on Nitrates
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.mean(data, d => +d['Nitrates'])
    }));
    
    // Sort the districts in ascending order based on average Nitrates
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerNitrates')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Nitrates']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Nitrates']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Nitrates']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Nitrates']}`)
