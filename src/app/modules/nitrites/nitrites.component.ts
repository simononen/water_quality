...
@Component({
  selector: 'app-nitrites',
  templateUrl: './nitrites.component.html',
  styleUrls: ['./nitrites.component.scss']
})
export class NitritesComponent implements AfterViewInit, OnDestroy {
  ...
  getWeatherData() {
    this._dataService.data?.subscribe(data => {
      ...
      if(this.data?.length) {
        this.maxPop = Math.max(...this.data.map((x:any) => x['Nitrites']), 0);
      }
  ...
  makeMarkers(map: any, dataSrc: any, maxPh: any): void {
    for (let i = 0; i < dataSrc.length; i++) {
      ...
      const circle = L.circleMarker([lat, lon], { radius: this.scaledRadius(item['Nitrites'], maxPh) });
  ...
  makePopup(data: any): string {
    return `` +
      ...
      <div>
        <span class="font-bold text-xs text-gray-500">Nitrites:</span>
        <span class="text-blue-400" [ngClass]="(data['Nitrites'] < 100) ? 'text-red-500' : 'text-red-500'"> ${ data['Nitrites'] } µs/cm </span> 
      </div>
      <div>
  ...
  createBarChart(data: any[]): void {
    ...
    const chartTitle = 'Total Nitrites by District';
    const xLabel = 'Districts';
    const yLabel = 'Total Nitrites';
    ...
    // Sort the data in ascending order based on Nitrites
    const districtAverages = data.map(d => ({
      district: d.District,
      average: d3.sum(data, d => +d['Nitrites'])
    }));
    
    // Sort the districts in ascending order based on average Nitrites
    districtAverages.sort((a: any, b: any) => a.average - b.average);
    
    const svg = d3.select('#chartContainerNitrites')
      .append('svg')
      .attr('width', svgWidth)
    ...
    const conductivityData = data.map(d => +d['Nitrites']);
    ...
    chart.selectAll('.bar')
      ...
      .attr('y', d => yScale(+d['Nitrites']))
      .attr('width', xScale.bandwidth())
      .attr('height', d => chartHeight - yScale(+d['Nitrites']))
      ...
      tooltip.style('opacity', 1)
        .html(`<strong>${d.District}</strong><br>Average Conductivity: ${d['Nitrites']}`)
